...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, DiffusionPipeline, UNet2DConditionModel,EulerDiscreteScheduler
...
def parse_args():
    ...
    parser.add_argument(
        "--validation_prompt", type=str, default="a man wearing a jacket",
    ...
    # lora args
    parser.add_argument("--use_peft", default=True, action="store_true", help="Whether to use peft to support lora")
...
def main():
    ...
    def tokenize_captions(examples, is_train=True):
        ...
        for caption in examples[caption_column]:
            if isinstance(caption, str):
                captions.append(args.prefix_name+caption)
            elif isinstance(caption, (list, np.ndarray)):
                # take a random caption if there are multiple
                captions.append(args.prefix_name+random.choice(caption) if is_train else caption[0])
            else:
                raise ValueError(
    ...
    def collate_fn(examples):
        ...
        pixel_values = pixel_values.to(memory_format=torch.contiguous_format).to(accelerator.device).float()
        input_ids = torch.stack([example["input_ids"] for example in examples]).to(accelerator.device)
...
#python3 finetune.py --config=configs/Barbie.json --output_dir=$path_to_save --sd_version=1.5 --threshold=2e-3 --lr_scheduler=cosine --progresive_iter=2500  --lambda_rank=0.0005
