...
class Sparse(torch.autograd.Function):
    @staticmethod
    def forward(ctx, param1,param2,mask):
        # print('forwad')
    ...
    def backward(ctx, grad_param1):
        ...
        return None,grad_param1[mask],None
def set_nested_attr(obj, attr_path, value):
    attrs = attr_path.split('.')
    for attr in attrs[:-1]:
        obj = getattr(obj, attr)
    obj._parameters[attrs[-1]]= value
class AdamW(Optimizer):
    ...
    def set_mask_param(self, threshhold):
        param_mask_groups = []
        trainable_params=[]
        for idx,param in enumerate(self.source_params):
            bool_mask=(param.abs()<threshhold)
            param_mask_groups.append(bool_mask)
            trainable_param=param[bool_mask].clone().detach().requires_grad_(True)
        ...
        return trainable_params,param_mask_groups
    ...
    def load_params(self,load_params):
        with torch.no_grad():
            for idx,param in enumerate(self.param_groups[0]['params']):
                try:
                    param[self.param_mask_groups[idx].bool()]=load_params[idx].to(param.device)
                except:
                    raise ValueError(f"require {self.param_mask_groups[idx].sum()} params in {idx} layers, but {len(load_params[idx])} params are provided.")
    ...
    def step(self, closure=None,unet_param=None):
        ...
        if self.inner_iter <= self.progressive_iter:
            ...
            current_mask = self.param_mask_groups_adaptive

        if self.lambda_rank!=0:
            with torch.enable_grad():
                random_id=random.choice(self.low_rank_ids)
...
def adamw(params: List[Tensor],
    ...
    func(params,
         ...
         param_mask_groups=param_mask_groups)
...
def _single_tensor_adamw(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        with torch.enable_grad():
            value=Sparse.apply(source_params[i].detach(),param,param_mask_groups[i])
            set_nested_attr(model, names[i], value)
